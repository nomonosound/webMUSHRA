...
function P835BakPage(_pageManager, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _mushraValidator, _errorHandler, _language) {
  this.isMushra = true;
...
P835BakPage.prototype.getName = function () {
  return this.pageConfig.name;
};

P835BakPage.prototype.init = function () {
  var toDisable;
  var td;
  var active;
...
P835BakPage.prototype.render = function (_parent) {
  var div = $("<div></div>");
  _parent.append(div);
  var content;
  ...
  for (i = 0; i < conditions.length; ++i) {
    td = $("<td class='spaceForSlider'> \
      <span><input type='range' name='"+ conditions[i].getId() + "' class='scales' value='5' min='1' max='5' step='0.01' data-vertical='true' data-highlight='true' style='display : inline-block; float : none;'/></span> \
    </td>");
...
P835BakPage.prototype.pause = function () {
  this.mushraAudioControl.pause();
};

P835BakPage.prototype.setLoopStart = function () {
...
P835BakPage.prototype.setLoopEnd = function () {
  var slider = document.getElementById('slider');
...
P835BakPage.prototype.btnCallbackReference = function () {
...
P835BakPage.prototype.surpressLoop = function () {
...
P835BakPage.prototype.btnCallbackCondition = function (_index) {
...
P835BakPage.prototype.renderCanvas = function (_parentId) {
  ...
  var xDrawingStart = $('#spaceForScale').offset().left - $('#spaceForScale').parent().offset().left + canvasContext.measureText(this.pageManager.getLocalizer().getFragment(this.language, 'distorted3') + "    3").width * 2.0;
  ...
  var xLetters = $("#refCanvas").width() + ($("#spaceForScale").width() + canvasContext.measureText(this.pageManager.getLocalizer().getFragment(this.language, 'noticeable3') + "    3").width) / 2.5;

  canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'noticeable5') + "    5", xLetters, YfirstLine);
  canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'noticeable4') + "    4", xLetters, prevScalesHeight * 0.25 + YfirstLine);
  canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'noticeable3') + "    3", xLetters, prevScalesHeight * 0.5 + YfirstLine);
  canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'noticeable2') + "    2", xLetters, prevScalesHeight * 0.75 + YfirstLine);
  canvasContext.fillText(this.pageManager.getLocalizer().getFragment(this.language, 'noticeable1') + "    1", xLetters, prevScalesHeight + YfirstLine);

};


P835BakPage.prototype.load = function () {
...
P835BakPage.prototype.save = function () {
  this.macic.unbind();
...
P835BakPage.prototype.store = function () {
